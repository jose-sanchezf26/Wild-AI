...
public class AnimalSpawner : MonoBehaviour
{
    ...
    public AnimalDataManager animalDataManager; // Referencia al AnimalDataManager

    void Start()
    {
        if (animalDataManager != null)
        {
            SpawnAnimals();
        }
        ...
        void SpawnAnimals()
        {
            foreach (var animal in animalsToSpawn)
            {
                for (int i = 0; i < animal.quantity; i++)
                {
                    ...
                    if (i == 0 && animalDataManager != null)
                    {
                        Animal animalO = animalObj.GetComponent<Animal>();
                        animalDataManager.animalsInfo.Add(new CreateAnimalData(animalO.animalName, animalO.heightRange, animalO.widthRange, animalO.densityFactor, animalO.possibleColors));
                    }
                }
            }
        }
